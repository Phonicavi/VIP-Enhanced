...
void Initalize_Topology()
{
    ...
    FILE *topo = fopen("Backhaul.topo","r");
...
inline void Read_File(int cur_node,int num)
{
    ...
    for (int i=0;i<20;++i) fscanf(read_file,"%d",&temp);
    ...
    for(i = 1; i <= NumOfNodes; ++i){
        if(tempmaps[start][i])
        {
            if(i == end)/*if the target*/
            {
                ...
                record[length] = i;
                ++length;
                if(length > Min_Len || length > NumOfNodes - 1)
                    return;
                tempmaps[start][i] = 0;
    ...
    while((n = TASK_ID++)<=NumOfNodes){
        ...
        // if (Cur_Time >100 &&  NodeArr[n].data_que[k].acc_buffer!=0)printf("%lf\n", NodeArr[n].data_que[k].acc_buffer);
           ...
           //  NodeArr[n].data_que[k].acc_buffer += (temp <= 1 ? temp : 0.5*(temp - 1)+1);
            ...
            NodeArr[n].data_que[k].acc_buffer += (1*shrink[n][k]);
